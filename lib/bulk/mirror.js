 ...
 * @param {object} client
 * @param {Array} nodes
 * @param {{ spaceKey:string, parentId:(string|null), execute:boolean, ops:Array, underNewParent?:boolean, readFile?:Function }} ctx
 */
  ...
  for (const node of nodes) {
    ...
    if (node.kind === 'folder') {
      ...
      if (existing) {
        ...
        if (execute) await client.updatePage(existing.id, node.title, readFile(node.path), 'markdown');
      } else {
