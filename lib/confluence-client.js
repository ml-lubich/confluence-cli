...
class ConfluenceClient {
   ...
   * exactly. Uses CQL `parent = ...` so folders are included. Returns
   * `{ id, title, type }` or null. Used by `mirror` for idempotency.
   */
  async findChildByTitle(parentId, title) {
    const cql = `parent = ${parentId} and title = "${this.escapeCql(title)}"`;
    const results = await this.search(cql, 25, true);
    return results.find((r) => r.title === title) || null;
  }

  /**
   * List ALL descendants of a root, of any content type (pages AND folders),
   * via CQL `ancestor = ...`. Unlike getAllDescendantPages (which only walks
   * /child/page), this sees folders and content nested under them. Returns
   * `[{ id, title, type }]`. Note: CQL is eventually consistent, so very
   * recently created content may lag by a few seconds.
   */
  async getDescendants(rootIdOrUrl) {
    const rootId = await this.extractPageId(rootIdOrUrl);
    const results = await this.search(`ancestor = ${rootId}`, 1000, true);
