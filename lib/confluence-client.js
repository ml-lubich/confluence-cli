...
class ConfluenceClient {
  ...
  /**
   * Move a page OR folder to a new parent using the dedicated move endpoint
   * (PUT /content/{id}/move/{position}/{targetParentId}).
   *
   * Preferred over movePage(): it works for folders (which have no body),
   * allows moving across spaces, and never re-writes the content body — so
   * macro-heavy pages cannot be mangled.
   *
   * @param {string} idOrUrl              - id or URL of the content to move
   * @param {string} targetParentIdOrUrl  - id or URL of the new parent
   * @param {string} [position='append']  - 'append' | 'before' | 'after'
   * @returns {Promise<{id:string,targetParentId:string,position:string,data:object}>}
   */
  async moveContent(idOrUrl, targetParentIdOrUrl, position = 'append') {
    const VALID_POSITIONS = ['append', 'before', 'after'];
    if (!VALID_POSITIONS.includes(position)) {
      throw new Error(
        `Invalid move position "${position}". Must be one of: ${VALID_POSITIONS.join(', ')}.`
      );
    }

    const id = await this.extractPageId(idOrUrl);
    const targetParentId = await this.extractPageId(targetParentIdOrUrl);

    const response = await this.client.put(
      `/content/${id}/move/${position}/${targetParentId}`
    );
  ...
  /**
   * List the DIRECT children of a container, of any content type (pages AND
   * folders), via GET /content/{id}/child?expand=page,folder. This endpoint is
   * immediately consistent (unlike CQL), so it is safe to call right after
   * creating content. Returns `[{ id, title, type }]`.
   */
  async getChildrenAllTypes(parentId) {
    const response = await this.client.get(`/content/${parentId}/child`, {
      params: { expand: 'page,folder', limit: 200 },
    });
    const data = response.data || {};
    const out = [];
    for (const type of ['folder', 'page']) {
      const results = (data[type] && data[type].results) || [];
      for (const r of results) {
        out.push({ id: String(r.id), title: r.title, type: r.type || type });
      }
    }
    return out;
  }
  ...
  /**
   * Find a direct child (page OR folder) of `parentId` whose title matches
   * exactly. Uses the immediately-consistent child endpoint so `mirror`
   * idempotency is reliable even right after a parent is created. Returns
   * `{ id, title, type }` or null.
   */
  async findChildByTitle(parentId, title) {
    const children = await this.getChildrenAllTypes(parentId);
    return children.find((c) => c.title === title) || null;
  }

  /**
   * List ALL descendants of a root, of any content type (pages AND folders), by
   * recursively walking the immediately-consistent child endpoint. Each item
   * carries its `depth` (root's direct children are depth 1) so callers can act
   * deepest-first. Returns `[{ id, title, type, depth }]`.
   *
   * This matters for deletion: deleting a container does NOT cascade-delete
   * child folders (they get re-parented to the space), so every item must be
   * deleted explicitly, children before parents.
   */
  async getDescendants(rootIdOrUrl, options = {}) {
    const maxDepth = options.maxDepth || 50;
    const rootId = await this.extractPageId(rootIdOrUrl);
    const out = [];
    const walk = async (id, depth) => {
      if (depth > maxDepth) return;
      const children = await this.getChildrenAllTypes(id);
  ...
  /**
   * Find content in a space by exact title (any depth). Returns `{ id, title,
   * type }` or null. Used by `mirror` to match top-level items.
   */
  async findContentByTitleInSpace(spaceKey, title) {
    const response = await this.client.get('/content', {
      params: { spaceKey, title, limit: 25 },
    });
    const results = response.data?.results || [];
    const match = results.find((p) => p.title === title) || null;
    return match ? { id: String(match.id), title: match.title, type: match.type || 'page' } : null;
  }
